...
const extracted = JSON.parse(readFileSync(join(outDir, 'extract.json'), 'utf8')) as ExtractedFloorplan
...
function overlaySvg(plan: ExtractedFloorplan, size: { width: number; height: number }): string {
  const maxAbs = extractMaxAbs(plan)
  const px = (x: number, y: number): [number, number] => overlayPoint(x, y, size, maxAbs)
